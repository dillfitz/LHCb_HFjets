                 ...
                 bool isData = false)
{
  ...
  extension_reco = TString("jpsieff_") + "reco" + Form("_ev_%d", NumEvts) + str_Mag + str_flavor + str_DTF + str_PID + str_GS + Form("_%d", dataset);

  TString extension, extension_eff, extension_read, extension_prefix;
  extension = TString("jpsieff_") + str_level + Form("_ev_%d", NumEvts) + str_Mag + str_flavor + str_DTF + str_PID + str_GS + Form("_%d", dataset);
  extension_eff = TString("hists/efficiency_truth") + Form("_ev_%d", NumEvts) + Form("_ptj_%d%d", int(ptMin), int(ptMax)) + Form("_eta_%.1f%.1f", etaMin, etaMax) + str_followHard + str_Mag + str_flavor + str_GS + Form("_%d", dataset);
  ...
  if (year == 9 && JetMeth != 9)
  {
    for (int i = 0; i < vec_datasets.size(); i++)
    {
      ...
      JpsiRecoTree->Add("hists/" + extension_read + ".root/JpsiRecoTree");
    }
  }
  else
  {
  ...
  // JpsiRecoTree->SetBranchAddress("Jpsi_Hlt2DiMuonDetachedHeavy_TOS", &Jpsi_Hlt2DiMuonDetachedHeavy_TOS);
  ...
  for (int ev = 0; ev < NumEvts; ev++)
  {
    ...
    bool TOS = (Jpsi_L0DiMuon_TOS || Jpsi_L0Muon_TOS) && Jpsi_Hlt1_TOS && Jpsi_Hlt2_TOS;
  ...
  for (int j1 = 0; j1 < ptJpsibinsize; j1++)
  {
    for (int j = 0; j < Jpsietabinsize; j++)
    {
      ...
      TFile *file_workspacenum = new TFile(Form("hists/workspacenum%d_", i) + extension_reco + ".root", "READ");
      TFile *file_workspacedenom = new TFile(Form("hists/workspacedenom%d_", i) + extension_reco + ".root", "READ");
      ...
      mu_num = new RooRealVar("mu_num", "mu", 3.097, 3.08, 3.1);
      sigma_num = new RooRealVar("sigma_num", "sigma", 0.01, 0.001, 0.1);
      alpha1_num = new RooRealVar("alpha1_num", "alpha1", 2., 0.1, 6.); // CHANGE
      alpha2_num = new RooRealVar("alpha2_num", "alpha2", 2., 0.1, 6.); // CHANGE
      p1_num = new RooRealVar("p1_num", "p1", 1., 0.1, 8.);             // CHANGE
      p2_num = new RooRealVar("p2_num", "p2", 1., 0.1, 8.);

      mu_denom = new RooRealVar("mu_denom", "mu", 3.097, 3.08, 3.1);
      sigma_denom = new RooRealVar("sigma_denom", "sigma", 0.01, 0.001, 0.1);
      alpha1_denom = new RooRealVar("alpha1_denom", "alpha1", 2., 1, 6.); // CHANGE
      alpha2_denom = new RooRealVar("alpha2_denom", "alpha2", 2., 1, 6.); // CHANGE
      ...
      RooRealVar *nbkg_num = new RooRealVar("nbkg_num", "fraction of background", 500, 0., 1000000);
      RooRealVar *nbkg_denom = new RooRealVar("nbkg_denom", "fraction of background", 500, 0., 1000000);

      RooRealVar *nsig_num = new RooRealVar("nsig_num", "fraction of background", 500, 0., 1000000);
      RooRealVar *nsig_denom = new RooRealVar("nsig_denom", "fraction of background", 500, 0., 1000000);
      ...
      Jpsimass_num.plotOn(xframe, Name("Jpsi_mass"));
      ...
      Jpsimass_denom.plotOn(xframe2, Name("Jpsi_mass"));

      model_denom->plotOn(xframe2, Name("model"), LineStyle(kSolid), LineColor(kRed), LineWidth(2));
      xframe2->Draw("SAME");
      ...
      // wnum->Print();
      wnum->writeToFile(Form("hists/workspacenum%d_", i) + extension + ".root");

      wdenom->import(*model_denom);
      // wdenom->import(JpsiMass);
      // wdenom->Print();
      wdenom->writeToFile(Form("hists/workspacedenom%d_", i) + extension + ".root");
    }
  }
  // }
  ican = 0;
  ...
  for (int i = 0; i < Jpsietabinsize; i++)
  {
    ...
    // legendptmass->AddEntry(h1_temp, Form("%.1f < #eta < %.1f GeV", Jpsieta_binedges[i], Jpsieta_binedges[i + 1]));
  }
  ...
  for (int i = 0; i < Jpsietabinsize; i++)
  {
    ...
    h1_temp->SetMinimum(0.9);
    h1_temp->SetTitle("; p_{T}^{HF} [GeV]; TISTOS/True MC");
    h1_temp->SetTitleOffset(1.2);
    h1_temp->GetXaxis()->CenterTitle(false);
  ...
  // auto legendptmass = new TLegend(0.45, 0.55, 0.75, 0.85);
  // legendptmass->SetTextSize(0.03);
  // legendptmass->SetBorderSize(0);
  // legendptmass->SetFillStyle(0);
  // legendptmass->SetFillColor(3);
