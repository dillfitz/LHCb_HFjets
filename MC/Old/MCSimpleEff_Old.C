...
void MCSimpleEff(int NumEvts = -1, int dataset = 91599,
    ...
    int  NumEvtsTrue0 = 6022748; // MC(08152023)
    int  NumEvtsTrue1 = 11501238; // MC(10192023) Both MU & MD
    int  NumEvtsTrue2 = 5754817; // MC(10192023) True MU
    int  NumEvtsTrue3 = 5746421; // MC(10192023) True MD
    
    // This should be the file from MCMakeVarTree
    
//    extension_read = TString("MCtree_") + str_level + TString("_ev_2634505") + str_flavor;
//    extension_read = TString("MCtree_") + str_level + TString("_ev_6022748") + str_flavor;
    
//    extension_read = TString("MCtree_") + str_level +  Form("_ev_%d", NumEvtsTrue1) + str_flavor;
//    extension_read = TString("MCtree_") + str_level + str_mag + Form("_ev_%d", NumEvtsTrue2) + str_flavor; 
...
//    TH1D *h1_num_efficiency_jetpt = new TH1D("num_efficiency_jetpt", "", 8 , 12.5, 250.0);
//    TH1D *h1_denom_efficiency_jetpt = new TH1D("denom_efficiency_jetpt", "",8 , 12.5, 250.0);
    ...
    TH1D *h1_jetpt_ratio = new TH1D("jet_pt_ratio", "; Jet pT ; (Reco Jet pT)/(True Jet pT)", 20, 0.0, 100.0);
    TH1D *h1_HFpt_reco_ext = new TH1D("HFpt_reco_ext", "", 20, 0.0, 100.);
    TH1D *h1_HFpt_truth_ext = new TH1D("HFpt_truth_ext", "", 20, 0.0, 100.);
    TH1D *h1_HFpt_ratio = new TH1D("HF_pt_ratio", "; HF pT ; (Reco HF pT)/(True HF pT)", 20, 0.0, 100.0);

    TH1D *h1_ratio_jet_12520 =  new TH1D("h1_ratio_jet_12520", "", 100, -4.0, 4.0);
    TH1D *h1_ratio_jet_2030 =  new TH1D("h1_ratio_jet_2030", "", 100, -4.0, 4.0);
    TH1D *h1_ratio_jet_3050 =  new TH1D("h1_ratio_jet_3050", "", 100,-4.0,4.0);
    TH1D *h1_ratio_jet_50100 =  new TH1D("h1_ratio_jet_50100", "", 100,-4.0,4.0);
    TH1D *h1_ratio_jet_100150 =  new TH1D("h1_ratio_jet_100150", "", 100,-4.0,4.0);

    TH1D *h1_ratio_HF_025 =  new TH1D("h1_ratio_HF_025", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_255 =  new TH1D("h1_ratio_HF_255", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_575 =  new TH1D("h1_ratio_HF_575", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_7510 =  new TH1D("h1_ratio_HF_7510", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_10125 =  new TH1D("h1_ratio_HF_10125", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_12515 =  new TH1D("h1_ratio_HF_12515", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_1520 =  new TH1D("h1_ratio_HF_1520", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_2030 =  new TH1D("h1_ratio_HF_2030", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_3050 =  new TH1D("h1_ratio_HF_3050", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_50100 =  new TH1D("h1_ratio_HF_50100", "", 100,-4.0,4.0);
    TH1D *h1_ratio_HF_100150 =  new TH1D("h1_ratio_HF_100150", "", 100,-4.0,4.0);
...
//        h1_jetpt_ratio->Divide(h1_jetpt_reco_ext, h1_jetpt_truth_ext);
//   h1_jetpt_ratio->Fill(meas_jet_pt/jet_pt );

        h1_HFpt_reco_ext->Fill(meas_HFmeson.Pt());
        h1_HFpt_truth_ext->Fill(HFmeson.Pt());
//        h1_HFpt_ratio->Divide(h1_HFpt_reco_ext, h1_HFpt_truth_ext);
//    h1_HFpt_ratio->Fill( ( meas_HFmeson.Pt() )/( HFmeson.Pt() ) );
        
        // Jet
        if(jet_pt > 12.5 && jet_pt < 20.0){
            h1_ratio_jet_12520->Fill(meas_jet_pt/jet_pt);
            
        }
        
        if(jet_pt > 20. && jet_pt < 30.0){
            h1_ratio_jet_2030->Fill(meas_jet_pt/jet_pt);
        }
        
        if(jet_pt > 30. && jet_pt < 50.0){
            h1_ratio_jet_3050->Fill(meas_jet_pt/jet_pt);
        }
        
        if(jet_pt > 50. && jet_pt < 100.0){
            h1_ratio_jet_50100->Fill(meas_jet_pt/jet_pt);
        }
        
        if(jet_pt > 100. && jet_pt < 150.0){
            h1_ratio_jet_100150->Fill(meas_jet_pt/jet_pt);
        }
        
        // HF
        if(HFmeson.Pt() > 0.0 && HFmeson.Pt() < 2.5 ){
            h1_ratio_HF_025->Fill(HF_pt/meas_HF_pt);
        }
        
        if(HFmeson.Pt() > 2.5 && HFmeson.Pt() < 5.0){
            h1_ratio_HF_255->Fill(HF_pt/meas_HF_pt);
        }
        
        if(HFmeson.Pt() > 5.0 && HFmeson.Pt() < 7.5){
            h1_ratio_HF_575->Fill(HF_pt/meas_HF_pt);
        }
        
        if(HFmeson.Pt() > 7.5 && HFmeson.Pt() < 10.0){
            h1_ratio_HF_7510->Fill(HF_pt/meas_HF_pt);
        }
        
        if(HFmeson.Pt() > 10.0 && HFmeson.Pt() < 12.5){
            h1_ratio_HF_10125->Fill(HF_pt/meas_HF_pt);
        }

        if(HFmeson.Pt() > 12.5 && HFmeson.Pt() < 15.0){
            h1_ratio_HF_12515->Fill(HF_pt/meas_HF_pt);
        }
        if(HFmeson.Pt() > 15.0 && HFmeson.Pt() < 20.0){
            h1_ratio_HF_1520->Fill(HF_pt/meas_HF_pt);
        }
        if(HFmeson.Pt() > 20.0 && HFmeson.Pt() < 30.0){
            h1_ratio_HF_2030->Fill(HF_pt/meas_HF_pt);
        }
        if(HFmeson.Pt() > 30.0 && HFmeson.Pt() < 50.0){
            h1_ratio_HF_3050->Fill(HF_pt/meas_HF_pt);
        }
        if(HFmeson.Pt() > 50.0 && HFmeson.Pt() < 100.0){
            h1_ratio_HF_50100->Fill(HF_pt/meas_HF_pt);
        }
        if(HFmeson.Pt() > 100.0 && HFmeson.Pt() < 150.0){
            h1_ratio_HF_100150->Fill(HF_pt/meas_HF_pt);
        }
        //
        /// Efficiencies, Response Matrixes, etc.
        //

        if (hasRecoHF && meas_jet_pt > 12.5 && meas_jet_eta > etaMin && meas_jet_eta < etaMax) {
        ...
        if (jet_pt > 12.5 && jet_pt < 20. && jet_eta > etaMin & jet_eta < etaMax) {
            h1_denom_efficiency_HFpt15->Fill(HFmeson.Pt());
        }

        if (jet_pt > 12.5 && jet_eta > etaMin & jet_eta < etaMax) {
...
//    if(jet_pt > ptMin && jet_pt < ptMax && jet_eta > etaMin & jet_eta < etaMax){
        if (jet_pt > 12.5 && jet_pt < 150. && jet_eta > etaMin & jet_eta < etaMax) {
...
//    if(jet_pt > ptMin && jet_pt < ptMax && jet_eta > etaMin & jet_eta < etaMax && meas_jet_pt > 15. && hasRecoHF){
        if (jet_pt > 12.5 && jet_pt < 150. && jet_eta > etaMin & jet_eta < etaMax && meas_jet_pt > 12.5 && hasRecoHF) {
        ...
        if (meas_jet_pt < 12.5 || meas_jet_eta < etaMin || meas_jet_eta > etaMax || !hasRecoHF) {
            continue;
        }
...
//    ccan[ican]->cd(7);
    ...
    TCanvas* c1 = new TCanvas("c1", "Jet Pt vs Mean", 1000, 800);
    c1->Divide(1,2);
    
    TH1D* jetHistograms[] = {h1_ratio_jet_12520, h1_ratio_jet_2030, h1_ratio_jet_3050, h1_ratio_jet_50100, h1_ratio_jet_100150};
    double jetBinCenters[] = {16.25, 25.0, 40.0, 75.0, 125.0};  // Midpoints of the pt ranges for jet
    double jetBinWidth[] = { 7.5, 10., 20., 50., 50};
    
    TH1D* HFHistograms[] = {h1_ratio_HF_025, h1_ratio_HF_255, h1_ratio_HF_575, h1_ratio_HF_7510, h1_ratio_HF_10125, h1_ratio_HF_12515,
                            h1_ratio_HF_1520, h1_ratio_HF_2030, h1_ratio_HF_3050, h1_ratio_HF_50100, h1_ratio_HF_100150};
    double HFBindEdges[] = {0.0, 2.5, 5.0, 7.5, 10., 12.5, 15.0, 20., 30.0, 50.0, 100., 150.};
    double HFBinCenters[11];
   
    // Calculate bin centers
    for (int i = 0; i < 11; i++) {
        HFBinCenters[i] = HFBindEdges[i] + (HFBindEdges[i+1] - HFBindEdges[i])/2.0;
    }

    TGraphErrors* graph_jet = new TGraphErrors(5);  // Assuming you have 5 pt ranges for jet
    TGraphErrors* graph_HF = new TGraphErrors(5);

    for (int i = 0; i < 5; i++) {
        double mean = jetHistograms[i]->GetMean();
        double meanError = jetHistograms[i]->GetMeanError();
        
        graph_jet->SetPoint(i, jetBinCenters[i], mean);
        graph_jet->SetPointError(i, jetBinWidth[i]/2, meanError);
    }
    
    for (int i = 0; i < 11 ; i++) {
        double mean_HF = HFHistograms[i]->GetMean();
        double meanError_HF = HFHistograms[i]->GetMeanError();
        
        double binWidth_HF = (HFBindEdges[i+1] - HFBindEdges[i]);
        
        graph_HF->SetPoint(i, HFBinCenters[i], mean_HF);
        graph_HF->SetPointError(i, 0, meanError_HF);
    }
    
    c1->cd(1);
    graph_jet->SetMarkerColor(kBlue);
    graph_jet->SetMarkerStyle(21);
    graph_jet->SetTitle(";Jet P_{T} (GeV/c); Mean Reco-Truth Ratio");
    graph_jet->Draw("AP");  // Draw graph_jet with axis
    graph_jet->Draw("samePE");
    graph_jet->GetYaxis()->SetRangeUser(0.4, 1.0);
    c1->Update();

    c1->cd(2);
    graph_HF->SetMarkerColor(kRed);
    graph_HF->SetMarkerStyle(21);
    graph_HF->SetTitle(";HF P_{T} (GeV/c); Mean Reco-Truth Ratio");
    graph_HF->Draw("AP");  // Draw graph_HF without axis
    graph_HF->Draw("samePE");
    graph_HF->GetYaxis()->SetRangeUser(0.95, 1.05);

//    TLegend *leg3 = new TLegend(0.70, 0.55, 0.80, 0.75);
//    gStyle->SetLegendTextSize(0.04);
//    leg3->SetTextFont(lhcbFont);
//    leg3->SetBorderSize(0);
//    leg3->AddEntry(graph_jet, " Jet ", "LPE");
//    leg3->AddEntry(graph_HF, " HF ", "LPE");
//    leg3->Draw("SAME");

    c1->Update();
    c1->SaveAs(plotextension + str_mag + "jet_HF_pt_ratios.pdf");
