...
//    TTree *BTree = (TTree *)fread.Get("BTree");
    ...
    BTree->SetBranchAddress("tr_pi_px", &tr_pi_px);
    BTree->SetBranchAddress("tr_pi_py", &tr_pi_py);
    BTree->SetBranchAddress("tr_pi_pz", &tr_pi_pz);
    BTree->SetBranchAddress("tr_pi_e", &tr_pi_e);
...
//            continue;
        ...
        TLorentzVector HFmeson, HFjet, recojet, mum , mup, pion, jpsi,  Bfromjet;
...
//            sigma2 = new RooRealVar("sigma2", "width of gaussians", 0.01, 0.001, 0.15); // CHANGE
...
//        RooAddPdf sig("sig", "Signal", RooArgList(dcbPdf_sig1, dcbPdf_sig2), RooArgList(*nsig1));
        ...
        RooRealVar *nsig = new RooRealVar("nsig", "fraction in signal", 10000, 850., 10000000.);
