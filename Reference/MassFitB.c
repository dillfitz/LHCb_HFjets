...
//            + str_flavor +  Form("_%d", dataset);
  ...
  TFile f("MassFitB.root", "RECREATE");
...
//  }
  // h1_mass->Scale(1./h1_mass->GetEntries());
   ...
   int nBins = 175;
   double mass_low = 5000.0;
   double mass_high = 5700.0;
  ...
  // 10  sigma2       1.78528e-02   1.77464e-04   1.00775e-04  -2.69847e+00
   ...
   RooRealVar mean("mean", "mean of gaussians", 5279., 5270., 5282.);
   RooRealVar sigma1("sigma1", "width of gaussians", 11.0, 8.0, 50);
   RooRealVar sigma2("sigma2", "width of gaussians", 21.0, 8.0, 50); //CHANGE
...
//   if(isData){
     mu_sig = RooRealVar("mu_sig","mu", 5276., 5260., 5282.);
...
//   }
   ...
   RooRealVar nsig1("nsig1", "fraction of component 1 in signal", 0.3, 0.0, 1.);
   ...
   // RooAddPdf("sig_nosec", "Signal", RooArgList(sig1_nosec, sig2_nosec), RooArgList(nsig1_nosec));
  ...
  // 10  width        2.62897e-02   4.95521e-05   2.87488e-04  -1.24508e+00
   RooRealVar mu("mu","mu", 5331.83e+00);
   ...
   RooRealVar m_arg("m_arg", "m", 5150.0, 5100.0, 5170.0);  // The peak position of the Argus function
  ...
  RooRealVar sigma_arg("sigma_arg", "sigma", 0.005, 0.001, 0.05);
  ...
  ///////////////////////////////////////////////////
   ...
   RooRealVar b1("b1", "scale", 15.0, 1.0, 40.0);
   //RooRealVar b2("b2", "scale", 0.015, 0.001, 0.07);
   RooRealVar s1("s1", "shift", 5140.0, 5120.0, 5160.0);
   ...
   RooRealVar a0_nosec("a0_nosec", "a0_nosec", 0.1, -1, 1.);
   RooRealVar a1_nosec("a1_nosec", "a1_nosec", 0.1, 0., 1.);
   ...
   RooRealVar nsig("nsig", "fraction in signal", 250, 0., 1000000.);


   // RooRealVar nsig2("nsig2", "fraction of component 2 in signal", 0.6, 0., 1.);
   RooRealVar nsig_nosec("nsig_nosec", "fraction in signal", 250, 0., 100000.);
   RooRealVar nbkg("nbkg", "fraction of background", 100, 0., 1000000.);
   RooRealVar nbkg_nosec("nbkg_nosec", "fraction of background", 100, 0., 1000000.);
   RooRealVar ntanh("ntanh", "fraction of background", 100, 0., 1000000.);
   RooRealVar nres("nres", "fraction of background", 100, 0., 1000000.);
   RooRealVar nres_nosec("nres_nosec", "fraction of background", 100, 0., 1000000.);
   // S a m p l e ,   f i t   a n d   p l o t   m o d e l
   // ---------------------------------------------------
   x.setRange("comb1", 5550, 5780);
   x.setRange("comb2", RooConst(mass_low), RooConst(mass_low + 100));
   x.setRange("tanh", 5050, 5200);
   x.setRange("noSec", 5180, 5600);
