...
void MassFit(int NumEvts = 10000, int dataset = 1510,bool isData = true,
  ...
  // 10  sigma2       1.78528e-02   1.77464e-04   1.00775e-04  -2.69847e+00
   ...
   RooRealVar mean("mean", "mean of gaussians", 5.279, 5.27, 5.282);
  ...
  ///////////////////////////////////////////////////
   ...
   RooRealVar nsig("nsig", "fraction in signal", 500, 0., 1000000.);
   ...
   if(isData) model =new RooAddPdf("model", "g1+g2+a", RooArgList(bkg, sig, tanhpdf, dcbPdf), RooArgList(nbkg, nsig, ntanh, nres));
...
model_nosec = new RooAddPdf("model_nosec", "g1+g2+a", RooArgList(bkg_nosec, sig_nosec, dcbPdf), RooArgList(nbkg_nosec, nsig_nosec, nres_nosec));
   ...
   double chi2;
   if(isData) chi2 = xframe->chiSquare(10);
   ...
   xframe->Draw("SAME");
   leg1->Draw("SAME");
   c->SaveAs("plots/" + extension + TString(".pdf"));
