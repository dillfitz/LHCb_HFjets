                  ...
                  bool SubtractGS = false)
{
  ...
  TChain *LundTree = new TChain("LundTree", "");
  ...
  if (year == 9 && JetMeth != 9)
  {
    for (int i = 0; i < vec_datasets.size(); i++)
    {
      ...
      LundTree->Add("hists/" + extension_read + ".root/LundTree");
    }
  }
  else
  {
  ...
  for (int i = 0; i < ptHFbinsize; i++)
  {
    ...
    // {
    TH3D *h3_ptktdR_tmp = new TH3D(Form("ptktdR%d", loc), "", dRbinsize, dR_binedges, ktbinsize, kt_binedges, ptbinsize, pt_binedges);
    vec_h3_ptktdR.push_back(h3_ptktdR_tmp);

    TH3D *h3_ptzdR_tmp = new TH3D(Form("ptzdR%d", loc), "", dRbinsize, dR_binedges, zbinsize, z_binedges, ptbinsize, pt_binedges);
  ...
  TH3D *h3_ptktdR_gluon = new TH3D("ptktdR_gluon", "", dRbinsize, dR_binedges, ktbinsize, kt_binedges, ptbinsize, pt_binedges);
  TH3D *h3_ptzdR_gluon = new TH3D("ptzdR_gluon", "", dRbinsize, dR_binedges, zbinsize, z_binedges, ptbinsize, pt_binedges);
  ...
  TH2D *h2_dRErad = new TH2D("dR_Erad", "", tr_Eradbinsize, tr_Erad_binedges, tr_dRbinsize, tr_dR_binedges);
  ...
  TH2D *h2_ktdR_gluon = new TH2D("kT_dR_gluon", "", tr_dRbinsize, tr_dR_binedges, tr_ktbinsize, tr_kt_binedges);
  TH2D *h2_zdR_gluon = new TH2D("z_dR_gluon", "", tr_dRbinsize, tr_dR_binedges, tr_zbinsize, tr_z_binedges);
  ...
  TH2D *h2_meas_dRErad = new TH2D("meas_dR_Erad", "", tr_Eradbinsize, tr_Erad_binedges, 80, 0.5, 6);
  ...
  TH1D *h1_erad = new TH1D("E_rad", "E Radiator", tr_Eradbinsize, tr_Erad_binedges);
  TH1D *h1_meas_erad = new TH1D("meas_E_rad", "E Radiator", tr_Eradbinsize, tr_Erad_binedges);
  ...
  TH1D *h1_frag_z = new TH1D("frag_z", "", fragzbinsize, fragz_binedges);
  TH1D *h1_frag_r = new TH1D("frag_r", "", 30, 0, 1);

  TH1D *h1_frag_jt_gluon = new TH1D("frag_jt_gluon", "", 30, 0, 12);
  TH1D *h1_frag_z_gluon = new TH1D("frag_z_gluon", "", fragzbinsize, fragz_binedges);
  TH1D *h1_frag_r_gluon = new TH1D("frag_r_gluon", "", 30, 0, 1);

  TH2D *h2_frag_z_jetpt = new TH2D("frag_z_jetpt", "", fragzbinsize, fragz_binedges, ptbinsize, pt_binedges);
  TH2D *h2_frag_z_jetpt_gluon = new TH2D("frag_z_jetpt_gluon", "", fragzbinsize, fragz_binedges, customptbinsize, custompt_binedges);
  ...
  for (int ev = 0; ev < NumEvts; ev++)
  {
    ...
    //   h1_mup_eta->Fill(mup.Eta());
    //   h1_mum_eta->Fill(mum.Eta());
    //   h1_K_eta->Fill(K.Eta());

    //   h1_mup_pt->Fill(mup.Pt());
    //   h1_mum_pt->Fill(mum.Pt());
    //   h1_K_pt->Fill(K.Pt());

    //   h1_meas_mup_eta->Fill(meas_mup.Eta());
    //   h1_meas_mum_eta->Fill(meas_mum.Eta());
    //   h1_meas_K_eta->Fill(meas_K.Eta());

    //   h1_meas_mup_pt->Fill(meas_mup.Pt());
    //   h1_meas_mum_pt->Fill(meas_mum.Pt());
    //   h1_meas_K_pt->Fill(meas_K.Pt());
    // }
    ...
    for (int idecl = 0; idecl < kts->size(); idecl++)
    {
      ...
      if (Hasbbbar)
      {
        h2_ktdR_gluon->Fill(logdR, logkt);
        h2_zdR_gluon->Fill(logdR, logz);
      }
      ...
      if (Hasbbbar)
      {
        h3_ptktdR_gluon->Fill(logdR, logkt, jet_pt);
        h3_ptzdR_gluon->Fill(logdR, logz, jet_pt);
      }

      ////// Non-perturbative cut //////
      if (logkt > log(LambdaQCD))
  ...
  SetRecoStyle(h1_GS_frac);
  SetTruthStyle(h1_FC_frac);
  ...
  for (int i = 0; i < ptHFbinsize; i++)
  {
    ...
    // {
    // int loc = i * HFetabinsize + j;
    vec_h3_ptktdR[loc]->Write(Form("h3_ptktdR%d", loc));
    vec_h3_ptzdR[loc]->Write(Form("h3_ptzdR%d", loc));
    // }
  }
  ...
  ccan[ican]->cd(1);
  h2_dRErad->GetYaxis()->SetRangeUser(0, 3);
  h2_dRErad->GetXaxis()->SetRangeUser(40, 400);
  h2_dRErad->SetStats(0);
  h2_dRErad->SetXTitle("E_{radiator}");
  h2_dRErad->SetYTitle("ln(R/#Delta R)");
  h2_dRErad->Draw("COLZ");
  ...
  h2_thetaErad->GetYaxis()->SetRangeUser(2, 5.5);
  h2_thetaErad->GetXaxis()->SetRangeUser(40, 400);
  ...
  ccan[ican]->cd(4);
  NormalizeHist(h1_frag_z);
  NormalizeHist(h1_frag_z_gluon);
  // stack_frag->Draw("P NOSTACK ");
  h1_frag_z->Draw("PE SAME");
  ...
  h1_jetpt_FC_frac->SetMaximum(1.05);
