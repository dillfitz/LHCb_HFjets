             ...
             float ptmax_user = 250)
{
    ...
    //tree_reco_ev_-1_eta_2.54.0_HF_ghost_0.5_b_91599.root
...
//    TString dir_deadcone("$LUND/BjetMisID/");
    ...
    LundTree->SetBranchAddress("tr_HF_px", &tr_HF_px);
    LundTree->SetBranchAddress("tr_HF_py", &tr_HF_py);
    LundTree->SetBranchAddress("tr_HF_pz", &tr_HF_pz);
    LundTree->SetBranchAddress("tr_HF_e", &tr_HF_e);
    ...
    LundTree->SetBranchAddress("pi_PIDK", &pi_PIDK);

    LundTree->SetBranchAddress("jet_pt", &jet_pt);
    LundTree->SetBranchAddress("jet_eta", &jet_eta);

    LundTree->SetBranchAddress("dphi", &dphi);
    LundTree->SetBranchAddress("nSV", &nSV);
    LundTree->SetBranchAddress("bmass_dtf", &bmass_dtf);
    LundTree->SetBranchAddress("chi2ndf_dtf", &chi2ndf_dtf);
    LundTree->SetBranchAddress("tau_dtf", &tau_dtf);
    LundTree->SetBranchAddress("isTrueBjet", &isTrueBjet);

    LundTree->SetBranchAddress("TOS", &TOS);
...
//        TFile *file_workspace = new TFile( Form("hists/workspace%d_", i) + extension_reco + ".root", "READ");
        ...
        if (UseDTF)
        {
            sigma_ratio = RooRealVar("sigma_ratio", "sigma_ratio", 1.52571 / 0.856549);
            mean = RooRealVar("mean", "mean of gaussians", 5.27966, 5.27, 5.282);
            width = RooRealVar("width", "width of gaussians", 0.002, 0.001, 0.3);
            sigma2 = RooRealVar("sigma2", "width of gaussians", 0.003, 0.001, 0.3); // CHANGE
        }
        else
        {
            sigma_ratio = RooRealVar("sigma_ratio", "sigma_ratio", 1.78528 / 3.15958);
            mean = RooRealVar("mean", "mean of gaussians", 5.279, 5.27, 5.282);
            width = RooRealVar("width", "width of gaussians", 0.002, 0.001, 0.05);
            sigma2 = RooRealVar("sigma2", "width of gaussians", 0.003, 0.001, 0.05); // CHANGE
        }
        ...
        mu = RooRealVar("mu", "mu", 5.32, 5.2, 5.4);
        alpha1 = RooRealVar("alpha1", "alpha1", 2., 0.01, 10.);
        alpha2 = RooRealVar("alpha2", "alpha2", 2., 0.01, 10.);
        // p1 = RooRealVar("p1", "p1", 1., 0.6, 4.);
        // p2 = RooRealVar("p2", "p2", 1., 0.6, 4.);
        p1 = RooRealVar("p1", "p1", 2.);
        p2 = RooRealVar("p2", "p2", 3.);
        ...
        RooRealVar a2("c2", "coefficient #2", -0.1, -1., -0.001);
        RooRealVar a1("exp_c", "exp_c", -2., -4, -0.1);
        ...
        RooRealVar nsig("nsig", "fraction in signal", 10000, 0., 10000000.);

        // RooRealVar nsig2("nsig2", "fraction of component 2 in signal", 0.6, 0., 1.);
        RooRealVar nsig_nosec("nsig_nosec", "fraction in signal", 500, 0., 1000000.);
        RooRealVar nbkg("nbkg", "fraction of background", 5000, 0., 1000000);
        RooRealVar nbkg_nosec("nbkg_nosec", "fraction of background", 200, 0., 1000000);
        RooRealVar ntanh("ntanh", "fraction of background", 200, 0., 1000000);
        RooRealVar nres("nres", "fraction of background", 1000, 0., 0.1 * nsig.getVal());
        // RooFormulaVar nres("nres", "resonant bkg", "0.0384*nsig", RooArgList(nsig)); // CHANGE
        RooRealVar nres_nosec("nres_nosec", "fraction of background", 200, 0., 1000000);
        ...
        w->import(B_mass);
        w->Print();
        w->writeToFile( extension_RootFilesMC + Form("workspace%d_", i) + extension + ".root");
